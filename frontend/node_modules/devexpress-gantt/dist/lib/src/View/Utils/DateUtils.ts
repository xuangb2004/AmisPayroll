...
export class DateUtils {
    ...
    static getNearestLeftTickTime(date: Date, rangeStartTime: number, tickTimeSpan: number, viewType: ViewType): number {
        ...
        const offsetDiff = DateUtils.getTimezoneOffsetDiff(new Date(rangeStartTime), date) * DateUtils.msPerMinute;

        const tickCountAtLeft = Math.floor((date.getTime() - rangeStartTime - offsetDiff) / tickTimeSpan);
    ...
    static getWeekStartDate(date: Date, firstDayOfWeek: number): Date {
        const dayOfWeek = date.getDay();
        let weekStartDayOfMonth = date.getDate() - dayOfWeek + firstDayOfWeek;
        if(dayOfWeek < firstDayOfWeek)
            weekStartDayOfMonth -= 7;
        return new Date(date.getFullYear(), date.getMonth(), weekStartDayOfMonth);
    }
    static getNextWeekStartDate(date: Date, firstDayOfWeek: number): Date {
