...
export class VisualizerManager extends BatchUpdatableObject implements IReadOnlyChangesListener,
    ...
    protected addShapeExtensionLines(shape: Shape, rect: Rectangle) {
        ...
        const eps = this.settings.gridSize / 2;
        if(x1 !== undefined && x2 !== undefined) {
            const distanceText = this.getViewUnitText(Math.abs(x1nc - x2nc));
            if(Math.abs(rect.center.y - sRect.center.y) < eps) {
                const segment = new Segment(new Point(x1, rect.center.y), new Point(x2, rect.center.y));
        ...
        if(y1 !== undefined && y2 !== undefined) {
            ...
            if(Math.abs(rect.center.x - sRect.center.x) < eps) {
                const segment = new Segment(new Point(rect.center.x, y1), new Point(rect.center.x, y2));
                this.extensionLinesVisualizer.addSegment(
