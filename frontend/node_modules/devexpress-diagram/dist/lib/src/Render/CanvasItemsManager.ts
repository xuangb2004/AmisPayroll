...
export class CanvasItemsManager extends CanvasManager implements IMouseOperationsListener,
    ...
    notifyTextInputStart(item: DiagramItem, _text: string, _position: Point, _size?: Size): void {
        const element = this.itemElements[item.key];
        DomUtils.addClassName(element, "text-input");
    }
    notifyTextInputEnd(item: DiagramItem, _captureFocus?: boolean): void {
        const element = this.itemElements[item.key];
    ...
    notifyActualZoomChanged(actualZoom: number): void {
    ...
    notifyViewAdjusted(_canvasOffset: Point): void { }
    ...
    createItemElements(key: ItemKey, primitives: SvgPrimitive<SVGElement>[], parent: SVGElement,
        className: string, type: MouseEventElementType): SVGElement {
    ...
    changeItemElements(primitives: SvgPrimitive<SVGElement>[], element: SVGElement, updateStructure: boolean): void {
    ...
    changeItemClassName(element: SVGElement, className?: string): void {
        if(className && element)
            this.changePrimitiveElement(new GroupPrimitive([], className), element);
    }
    removeItemElements(element: SVGElement): void {
        if(element && element.parentNode)
            element.parentNode.removeChild(element);
    }
    removeItemCustomContent(element: SVGElement, primitives: SvgPrimitive<SVGElement>[]): void {
    ...
    moveItemElements(parent: SVGElement, element: SVGElement, sibling?: SVGElement): void {
        if(element && parent)
            if(sibling)
    ...
    changeItemChildrenVisibility(element: SVGElement, expanded: boolean): void {
    ...
    getItemGroupContainer(zIndex: number, parentContainerKey?: ItemKey): SVGElement {
        ...
        if(this.itemGroupContainers[key] === undefined || this.itemGroupContainers[key][zIndex] === undefined) {
            ...
            const nextSiblingZIndex = Object.keys(this.itemGroupContainers[key]).map(z => +z).sort((a, b) => a - b).filter(z => z > zIndex)[0];
    ...
    getItemSelectorGroupContainer(zIndex: number, parentContainerKey?: ItemKey): SVGElement {
        ...
        if(this.itemSelectorGroupContainers[key] === undefined || this.itemSelectorGroupContainers[key][zIndex] === undefined) {
            ...
            const nextSiblingZIndex = Object.keys(this.itemSelectorGroupContainers[key]).map(z => +z).sort((a, b) => a - b).filter(z => z > zIndex)[0];
