...
import { RTree } from "../../../../Layout/Utility/RTree";
...
export interface IConnectorRoutingContext<TPoint extends Point> {
    ...
    getIntersectedItems(point: TPoint, margin: number): DiagramItem[];
...
export class IntersectingItemsByPointsContext<TPoint extends Point> {
    protected trees: {[margin: number]: RTree<DiagramItem>} = {};
    constructor(protected routingModel: IConnectorRoutingModel) { }

    getItems(point: Point, margin: number): DiagramItem[] {
        return this.getOrCreateTree(margin).search(point);
    }

    private getOrCreateTree(margin: number): RTree<DiagramItem> {
        if(!this.trees[margin]) {
            this.trees[margin] = new RTree<DiagramItem>();
            const items = this.routingModel.getItems();
            for(let i = 0, item: DiagramItem; item = items[i]; i++)
...
export class RightAngleConnectorRoutingContext implements IConnectorRoutingContext<Point> {
    ...
    getIntersectedItems(point: Point, margin: number): DiagramItem[] {
        return this.intersectedItemsByPointsContext
            .getItems(point, margin)
            .filter(item => !this.isIgnorableItem(item));
    }
    ...
    processIntersection(): void {
        const shapes = this.routingModel.getItems();
        if(shapes)
            shapes.forEach(s => {
                const key = s.key;
    ...
    private hasIntersectedItemsByPoint(point : Point, secondPoint : Point, connectionItem : DiagramItem) : boolean {
        return this.getIntersectedItems(point, 0)
            .some(i => i.key !== connectionItem.key && !i.rectangle.containsPoint(secondPoint));
    }
    ...
    private createExtendedShapesBounds() : Rectangle[] {
        return this.routingModel.getItems().map(i => i.rectangle.clone().inflate(this.routingModel.shapeMargins));
    }
