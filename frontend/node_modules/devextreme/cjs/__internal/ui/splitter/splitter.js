...
class Splitter extends _collection_widget.default {
    ...
    _updateItemsRestrictions(currentItem) {
        ...
        if (void 0 !== currentItem) {
            items.forEach(item => {
                if (item !== currentItem && false !== item.visible && true !== item.collapsed) {
                    const minSizeRatio = (0, _layout.convertSizeToRatio)(item.minSize, elementSize, handlesSizeSum);
                    otherPanesMinSizeSum += minSizeRatio ?? 0
                }
            })
        }
        items.forEach(item => {
            const sizeRatio = (0, _layout.convertSizeToRatio)(item.size, elementSize, handlesSizeSum);
            const minSizeRatio = (0, _layout.convertSizeToRatio)(item.minSize, elementSize, handlesSizeSum);
            const userMaxSize = (0, _layout.convertSizeToRatio)(item.maxSize, elementSize, handlesSizeSum);
            let effectiveMaxSize = userMaxSize;
            if (item === currentItem) {
                const maxFromOtherMinSizes = 100 - otherPanesMinSizeSum;
                effectiveMaxSize = (0, _type.isDefined)(userMaxSize) ? Math.min(userMaxSize, maxFromOtherMinSizes) : maxFromOtherMinSizes
            }
