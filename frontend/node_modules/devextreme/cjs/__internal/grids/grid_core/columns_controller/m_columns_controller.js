...
class ColumnsController extends _m_modules.default.Controller {
    ...
    _getIndexedColumns(columns) {
        ...
        columns.forEach(column => {
            ...
            if (isVisible && !isInGroupPanel) {
                ...
                let targetIndex = visibleIndex ?? "undefined";
                let indexedColumns;
                if ((0, _type.isDefined)(visibleIndex) && visibleIndex < 0) {
                ...
                if (parentBandColumns.length) {
                    targetIndex = (0, _m_columns_controller_utils.numberToString)(targetIndex, columnDigitsCount);
                    for (let i = parentBandColumns.length - 1; i >= 0; i -= 1) {
                        const {
                            visibleIndex: parentVisibleIndex
                        } = parentBandColumns[i];
                        targetIndex = `${(0,_m_columns_controller_utils.numberToString)(parentVisibleIndex,columnDigitsCount)}${targetIndex}`
                    }
                }
    ...
    _getVisibleColumnsFromIndexed(_ref) {
        ...
        }(0, _m_columns_controller_utils.processExpandColumns)(result[0], expandColumns, _const3.DETAIL_COMMAND_COLUMN_NAME, firstExpandColumnIndex, rowCount);
        (0, _m_columns_controller_utils.processExpandColumns)(result[0], expandColumns, _const3.GROUP_COMMAND_COLUMN_NAME, firstExpandColumnIndex, rowCount);
    ...
    updateSortingGrouping(dataSource, fromDataSource) {
        ...
        const updateSortGroupParameterIndexes = function(columns, sortParameters, indexParameterName) {
            const referencedGroupValues = columns.filter(column => (0, _type.isString)(column.calculateGroupValue)).map(column => column.calculateGroupValue);
            (0, _iterator.each)(columns, (_, column) => {
                const isReferencedAsGroupValue = "groupIndex" === indexParameterName && referencedGroupValues.some(groupValue => column.dataField === groupValue || column.name === groupValue);
        ...
        if (dataSource) {
            ...
            if (!that._columns.length) {
                (0, _iterator.each)(groupParameters, (_, group) => {
                    that._columns.push(group.selector)
                });
                (0, _iterator.each)(sortParameters, (_, sort) => {
