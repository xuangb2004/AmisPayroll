...
class ValidatingController extends _m_modules.default.Controller {
    ...
    resetValidationStateForChanges(changes) {
        if (!(null !== changes && void 0 !== changes && changes.length)) {
            return
        }
    ...
    _removeValidationData(key) {
        var _this$_validationStat;
        if (!(null !== (_this$_validationStat = this._validationState) && void 0 !== _this$_validationStat && _this$_validationStat.length)) {
            return
        }
        ...
        this._validationState = this._validationState.filter(data => isObjectKeyHash ? !(0, _common.equalByValue)(data.key, key) : data.key !== key)
    }
    ...
    validate(isFull) {
        ...
        if (isFull) {
            ...
            (0, _iterator.each)(changes, (index, _ref2) => {
                let {
                    type: type,
                    key: key
                } = _ref2;
        ...
        if (needCreateValidator) {
            ...
            const validator = new _validator.default($container || (0, _renderer.default)("<div>"), {
                ...
                dataGetter() {
                    ...
                    return {
                        data: change ? (0, _array_utils.createObjectWithChanges)(oldData, change.data) : Object.assign({}, oldData ?? parameters.data),
                        column: column
                    }
                },
    ...
    isCurrentValidatorProcessing(_ref3) {
        let {
            rowKey: rowKey,
    ...
    updateCellValidationResult(_ref4) {
        let {
            rowKey: rowKey,
            columnIndex: columnIndex,
    ...
    removeCellValidationResult(_ref6) {
        let {
            change: change,
            columnIndex: columnIndex
        } = _ref6;
    ...
    cancelCellValidationResult(_ref7) {
        let {
            change: change,
            columnIndex: columnIndex
        } = _ref7;
...
const validatingEditingExtender = Base => class extends Base {
    ...
    _updateRowAndPageIndices() {
        ...
        (0, _iterator.each)(that.getChanges(), (_, _ref0) => {
            let {
                key: key,
                type: type
            } = _ref0;
    ...
    _shouldResetValidationState() {
        ...
        if (isCellEditMode) {
            ...
            return 0 === columnsWithValidatingEditors.length
        }
        return true
    }
...
const validatingEditorFactoryExtender = Base => class extends Base {
    ...
    _getCurrentFocusElement($focus) {
        if (this._editingController.isEditing()) {
            const selector = [_const.EDITORS_INPUT_SELECTOR, _const.EDITORS_TEXTAREA_SELECTOR].join(", ");
