    ...
    alignIconsWithEditors() {
        ...
        $groups.toArray().forEach(groupElement => {
            ...
            const firstSimpleItemElement = Array.from(itemElements).find(itemElement => {
                const isGroup = null !== itemElement.querySelector(`.${CLASSES.formItem}`);
                const isIcon = null !== itemElement.querySelector(`.${CLASSES.formIcon}`);
                return !isGroup && !isIcon
            });
            if (!firstSimpleItemElement || !iconElement) {
                return
            }
            const hasTopLabel = null !== firstSimpleItemElement.querySelector(`.${CLASSES.labelTop}`);
            const hasInnerLabel = !hasTopLabel && null !== firstSimpleItemElement.querySelector(`.${CLASSES.label}`);
